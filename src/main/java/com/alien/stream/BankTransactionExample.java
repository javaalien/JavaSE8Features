...
public class BankTransactionExample {
	...
	public static void main(String[] args) {
		...
		System.out
				.println(maxTransaction.map(BankTransaction::getAccNumber).orElse("No transactions done on : " + when));

		// find the minimum individual transaction done today
		Optional<BankTransaction> minTransaction = getSmallestTransaction(when);

		System.out.println(minTransaction.map(BankTransaction::getAccNumber).orElse("No transactions on : " + when));
	...
	private static Optional<BankTransaction> getSmallestTransaction(LocalDate when) {
		...
		return transactions.stream().filter(tx -> tx.getDate().equals(when)).min(byAmount);
	}

	private static Optional<BankTransaction> getBiggestTransaction(LocalDate when) {
		Comparator<BankTransaction> byAmount = Comparator.comparing(BankTransaction::getAmount);

		return transactions.stream().filter(tx -> tx.getDate().equals(when)).max(byAmount);
	}

	private static long getTotalTransactionsToday() {

		return transactions.stream().filter(tx -> tx.getDate().equals(LocalDate.now())).count();
	}

	private static List<BankTransaction> getTransactions() {
		return Arrays.asList(new BankTransaction("123", BigDecimal.valueOf(250.00), LocalDate.now()),
